...
interface MarketSidebarProps {
  /**
   * default = persistent desktop sidebar
   * drawer = mobile drawer version (no header, slightly larger targets)
   */
  variant?: "default" | "drawer";
  onItemClick?: () => void;
}

export const MarketSidebar: FC<MarketSidebarProps> = ({
  variant = "default",
  onItemClick,
}) => {
  ...
  const containerClasses =
    variant === "drawer"
      ? "flex h-full w-full flex-col bg-[#0A0A0A] px-3 py-3 text-sm text-zinc-200"
      : "flex h-full w-56 flex-col border-r border-zinc-800 bg-[#0A0A0A] px-3 py-4 text-sm text-zinc-200";
        ...
        {NAV_ITEMS.map(({ ticker, label, href, Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              onClick={onItemClick}
              className={`flex w-full items-center justify-between rounded px-2 ${
                variant === "drawer" ? "py-2" : "py-1"
